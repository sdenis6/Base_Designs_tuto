...
\newline \newline
A webserer generator is available in the /oscimpDigital/app/tools/webserver\_generator repository, and build a standard webserver using the same My\_super\_design.xml file than the one used by the module generator :
...
For example, the calculation of $10$ coefficients ($9th$ order filter) on $16~bits$ ($\times 2^{16}$) for a lowpass filter with a cutoff frequency at the half range ($0.5$), gives the following output: 

\vspace{-0.1cm}
\begin{lstlisting}
octave:2> COEFF = transpose(int16(fir1(9,0.5)*2**16)))
COEFF =
257
-793
-2674
...
\vspace{-0.1cm}
\begin{lstlisting}
octave:2> figure(1); freqz(int16(fir1(54,0.048)*2**16),1,1024,125e6)
\end{lstlisting}
\vspace{-0cm}

\begin{figure}[!h!tb]
	\begin{center}
		\includegraphics[width=12cm,trim={1.5cm 6.9cm 1.5cm 7cm}, clip]{curves/lp55co.pdf}
...
\vspace{-0.1cm}
\begin{lstlisting}
octave:3> figure(2); freqz(int16(fir1(14,0.048)*2**16),1,1024,125e6)
\end{lstlisting}
\vspace{-0cm}
...
\vspace{-0.1cm}
\begin{lstlisting}
octave:4> figure(3); freqz(int16(fir1(54,[0.08 0.4 0.64 0.8],'stop')*2**16),1,1024,125e6)
\end{lstlisting}
\vspace{-0cm}
...
Then an example of python script to load one FIR with $55$ coefficients stored in the My\_FIR\_coefficients.dat file is :
\vspace{+0.8cm}
\begin{lstlisting}[language=Python]
#!/usr/bin/env python
...
liboscimp_fpga.fir_send_confSigned('/dev/MY_FIR_FILE', 'My_FIR_coefficients.dat', 55)
\end{lstlisting}
\vspace{0.5cm}
...
The filtering is performed using a FIR, explication garder 1 bande latérale...
...
\begin{center}
	\begin{tabular}{|>{\centering\arraybackslash}m{.3\linewidth} | >{\centering\arraybackslash}m{.3\linewidth} |}
		...
		\hline
		shiftReal\_0&Data in size: $32~bits$ \newline Data out size: $19~bits$\\
		\hline
		expanderReal\_0&Data in size: $19~bits$ \newline Data out size: $14~bits$\\
		\hline
	\end{tabular}
\end{center}
...
\begin{itemize}
	...
	\item Mix of a sine signal and the demodulation signal (i.e. not a demodulation)
